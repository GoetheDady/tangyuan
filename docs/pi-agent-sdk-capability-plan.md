...
> 本文记录元宵对 Pi Agent SDK 能力的理解、取舍和支持顺序。
>
...
- 默认 `agentId` 是 `yuanxiao`。
- Pi SDK 的 `cwd` 使用 `~/.yuanxiao/agents/yuanxiao`。
- v1 不创建额外 `workspace/` 目录。
- `bootstrap.md`、`soul.md`、`user.md`、`memory/`、`skills/` 都属于该 Agent。
- 工具默认以当前 Agent 的 `cwd` 为工作目录；这不是强制路径隔离。
- 后续多 Agent 使用 `~/.yuanxiao/agents/<agentId>`，每个 Agent 有独立 `cwd`、`soul.md`、`user.md`、`memory/` 和 `skills/`。

首次使用流程：

1. 应用创建 `~/.yuanxiao/agents/yuanxiao/bootstrap.md`。
...
- Agent 更新 `soul.md` 和 `user.md` 时直接通过 Pi SDK 工具修改文件，不先经过元宵自定义写入逻辑。
...
元宵必须能通过 `PiSdkDriver` 创建真实 Pi Agent SDK 会话。会话创建发生在 Electron Main 侧，不发生在 Renderer。
...
Pi SDK 的 `subscribe()` 事件必须转成元宵事件，UI 不理解 SDK 原始事件结构。
...
v1 使用 Pi SDK 原生 session 持久化作为会话历史的真实来源。元宵自己的 JSON 只保存 UI 索引、摘要和补充元数据。
...
- 元宵不复制完整 transcript，除非 SDK 无法稳定读取消息。
- 元宵 JSON 只保存会话列表展示和产品补充字段。
...
- `bash` 默认禁用。
- 使用 `cwd = ~/.yuanxiao/agents/yuanxiao`。
...
10. 映射 SDK 流式事件到元宵事件。
11. 实现取消运行。
12. 实现 Pi SDK session 持久化与元宵 session index。
