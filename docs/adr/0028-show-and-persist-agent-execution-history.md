...
聊天主界面的消息流除对话消息外，还展示并持久化与 Agent 回复关联的执行历史，包括 Runtime 提供的 thinking 原文、中间文字和工具步骤摘要。执行历史在运行期间默认展开，最终回复确认后按 Pencil 规则自动收起，并在完成、失败、中断或重新打开会话后仍可回看；工具原始参数和完整输出不进入 Renderer 展示。
