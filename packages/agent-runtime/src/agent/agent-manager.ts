 ...
 * 不持有独立状态，编排 AgentLifecycleModule 与 RuntimeSnapshotStore。
 */
export class AgentManager {
  private readonly agents: AgentLifecycleModule
   ...
   * @throws 当 Agent 模块创建失败时，Promise 会 reject。
   */
  async create(displayName: string): Promise<AgentSummary> {
    const summary = await this.agents.createAgent(displayName)
   ...
   * @throws 当 Agent 模块更新失败时，Promise 会 reject。
   */
  async updateConfig(request: UpdateAgentConfigRequest): Promise<AgentSummary> {
    const summary = await this.agents.updateAgentConfig(request.agentId, {
   ...
   * @throws 当 Agent 模块归档失败时，Promise 会 reject。
   */
  async archive(agentId: string): Promise<AgentSummary> {
    const summary = await this.agents.archiveAgent(agentId)
    await this.snapshotStore.reload()
    return summary
  }
   ...
   * @throws 当 Agent 模块恢复失败时，Promise 会 reject。
   */
  async recover(agentId: string): Promise<AgentSummary> {
    const summary = await this.agents.recoverAgent(agentId)
    await this.snapshotStore.reload()
    return summary
  }
   ...
   *
   * @returns 对账报告。
   * @throws 当 Agent 模块对账失败时，Promise 会 reject。
   */
  async reconcileDirectories(): Promise<{
    agents: AgentSummary[]
    unclaimedDirectories: UnclaimedDirectory[]
  }> {
    return this.agents.reconcileAgentDirectories()
  }
   ...
   * @throws 当 Agent 模块认领失败时，Promise 会 reject。
   */
  async claimDirectory(
    agentId: string,
    displayName: string,
  ): Promise<AgentSummary> {
    const summary = await this.agents.claimAgentDirectory(agentId, displayName)
    await this.snapshotStore.reload()
    return summary
  }
   ...
   * @throws 当 Agent 模块重建失败时，Promise 会 reject。
   */
  async rebuildYuanxiaoHome(): Promise<AgentSummary> {
    const summary = await this.agents.rebuildYuanxiaoHome()
    await this.snapshotStore.reload()
    return summary
  }
}
