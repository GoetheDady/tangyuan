...
export function SettingsProviderPage(): React.JSX.Element {
  ...
  const unconfiguredProviders = useMemo(
    () =>
      runtime?.providers.filter(
        (p) => !configuredProviderIds.includes(p.providerId),
      ) ?? [],
  ...
  if (isLoading) {
    return (
      <div className="text-body text-muted-foreground">正在加载配置...</div>
    )
  }
      ...
      {configuredProviderIds.length === 0 ? (
        ...
        <div className="space-y-2">
          {configuredProviderIds.map((providerId) => {
            const displayName =
              runtime?.providers.find((p) => p.providerId === providerId)
                ?.displayName ?? providerId
            const auth = runtime?.configuredProviders[providerId]
            const isDeleting =
              viewState.mode === 'delete' && viewState.providerId === providerId
