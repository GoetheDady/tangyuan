...
test('启动时恢复最后激活的自定义 Agent 分叉会话', async ({ page }) => {
  ...
  await expect(
    page.getByRole('button', { name: '切换到 Agent 研究助手' }),
  ).toHaveAttribute('aria-current', 'page')
  await expect(
    page.getByRole('treeitem', { name: /研究主线/ }),
  ).toHaveAttribute('aria-level', '1')
  await expect(
    page.getByRole('treeitem', { name: /研究分叉/ }),
  ).toHaveAttribute('aria-level', '2')
  await expect(page.getByText('分叉自「研究主线」')).toBeVisible()
  await expect(page.getByRole('combobox', { name: '模型' })).toHaveText(
    'Claude Sonnet 4.5',
  )
})
