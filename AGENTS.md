...
- **不保留向后兼容**：删除废弃路径，而非添加兼容层、fallback 或迁移逻辑。
- **最简实现**：选择完全满足当前需求的最简方案。避免推测性抽象、配置项和间接层。
- **分层生长**：从最小的端到端可工作版本出发，在已可用的产品上逐步叠加新能力。绝不用未完成的复杂性换掉能跑的产品。
- **模块化**：保持组件模块化，关注点清晰分离。
- **优先用成熟库**：当成熟、维护良好的库能降低整体复杂度或提升可靠性时，优先使用；不要无缘无故重新实现常见功能。
- **依赖现有包**：先充分利用项目已有依赖，再自行实现或引入新包。在查阅其文档和类型之前，不要假设某个库缺少某个能力。
- **长期架构决策**：为长远做架构决策。不接受仅能应对当下、预期之后要替换的权宜之计。
